...
describe("transactions", () => {
  ...
  test("should not create case when audit logging fails", async () => {
    ...
    const expectedCase = await models.cases.findOne({
      where: { createdBy: "test_transaction_user" }
    });

    expect(expectedCase).toBeNull();

    await models.civilian.destroy({
      where: { firstName: "test_transaction_civilian" }
    });
