...
describe('getCaseHistory', () => {
    ...
    test('should return case audits in order of created at desc', async () => {
        const dataChangeAudit1 = await createDataChangeAudit(caseId, '2017-01-31T13:00Z');
        const dataChangeAudit2 = await createDataChangeAudit(caseId, '2018-01-31T08:00Z');
        const dataChangeAudit3 = await createDataChangeAudit(caseId, '2018-01-31T06:00Z');
    ...
    test('should return the properties needed for display', async () => {
        const dataChangeAudit = await createDataChangeAudit(caseId, '2017-01-31T13:00Z');
        await getCaseHistory(request, response, next);
    ...
    const createDataChangeAudit = async (caseId, createdAt) => {
        const dataChangeAuditAttributes = new DataChangeAudit.Builder().defaultDataChangeAudit()
            .withId(undefined).withModelName('case').withModelId(caseId).withCaseId(caseId)
            .withAction(DATA_UPDATED).withChanges({}).withUser('bob').withCreatedAt(createdAt);
        return await models.data_change_audit.create(dataChangeAuditAttributes);
    };

    const auditChangeProperties = dataChangeAudit => {
        return _.pick(dataChangeAudit, ['id', 'caseId', 'action', 'modelName', 'modelId', 'changes', 'user', 'createdAt']);
    };
});
