...
describe('removeUserAction request', () => {
    ...
    test('should remove a user action', async () => {
        ...
        const userActionToCreate = new UserAction.Builder().defaultUserAction().withCaseId(createdCase.id).build()

        const createdUserAction = await models.user_action.create(userActionToCreate)

        await request(app)
            .delete(`/api/cases/${createdCase.id}/recent-activity/${createdUserAction.id}`)
            ...
            .then (response => {
                ...
                expect(currentCase).toEqual(expect.objectContaining({
                    recentActivity: [],
