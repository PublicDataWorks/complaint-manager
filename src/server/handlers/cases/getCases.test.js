...
describe("getCases", () => {
  ...
  describe("should sort cases", () => {
    const p1 = { caseId: 1, lastName: "Grant", firstName: "Test" };
    const p2 = { caseId: 2, lastName: "Zeta", firstName: "Test" };
    const cases = [
      {
        id: 1,
        status: INITIAL,
    ...
    `(
      ...
      async ({ sortBy, sortDirection, cases }) => {
        await request(app)
          ...
          .expect(200)
          .then(resp => {
            expect(map(resp.body.cases, "id")).toEqual(map(cases, "id"));
          });
      }
    );
  });
    ...
    const cases = [
      {
        id: 1,
        complainantCivilians: [
          { caseId: 1, lastName: "A", firstName: "Bravo" }
        ],
        complainantOfficers: [
          { caseId: 1, roleOnCase: COMPLAINANT, lastName: "Z" }
        ]
      },
      {
        id: 2,
        complainantCivilians: [
          { caseId: 2, lastName: "B", firstName: "Charles" }
        ]
      }
    ].map(mockCase);
