...
test('should update case status and recent activity in the db after user action edited', async () => {
    ...
    const updatedCase = await models.cases.find({where: {id: createdCase.id}})
    ...
    const updatedRecentActivity = await models.user_action.findAll({where: {caseId: createdCase.id}})
    expect(updatedRecentActivity).toEqual(expect.arrayContaining([
        expect.objectContaining({
