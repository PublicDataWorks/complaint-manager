...
describe("getReferralLetterPreview", function () {
  ...
  describe("with unknown user", () => {
    test("should have a null sender if user does not have a signature on file", async () => {
      ...
      expect(response._getData().addresses.sender).toBeFalsy();
    });
  });

  describe("with expected sender", () => {
    beforeEach(() => {
      request = httpMocks.createRequest({
        method: "GET",
        headers: {
    ...
    describe("in letter in progress status", () => {
      ...
      test("it renders the accused officers", async () => {
        ...
        const knownOfficer = await models.officer.create(
          knownOfficerAttributes,
          {
            auditUser: "someone"
          }
        );
      ...
      describe("snapshotTests", function () {
        ...
        beforeEach(async () => {
          ...
          const caseOfficerAllegationAttributes =
        ...
        test("ensures auditDataAccess is called with correct values", async () => {
          ...
          const expectedAuditDetails = {
            ...
            howDidYouHearAboutUsSource: {
              attributes: expect.arrayContaining(
                Object.keys(
                  models.how_did_you_hear_about_us_source.rawAttributes
                )
              ),
