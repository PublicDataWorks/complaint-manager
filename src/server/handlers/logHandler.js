...
const generateColor = (foregroundColor = foregroundColors.WHITE, backgroundColor = backgroundColors.BLACK) => string => {
  return "\x1b[" + foregroundColor + ";" + backgroundColor + "m" + string + "\x1b[0m";
};
...
const transportFormat = ({ level, message, timestamp, label }) => {
  return `${magenta(timestamp)} [${yellow(label)}] ${green(level)}: ${cyan(message)}`;
};
