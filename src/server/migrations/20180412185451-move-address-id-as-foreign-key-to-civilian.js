...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.sequelize.transaction(async transaction => {
      ...
      const addresses = await queryInterface.sequelize.query(query, {
        transaction
      });
      ...
      if (addresses[1].rowCount > 0) {
        addresses[0].map(async address => {
          query = `UPDATE civilians SET address_id=${
            address.id
          } where civilians.id=${address.civilian_id};`;
  ...
  down: async (queryInterface, Sequelize) => {
    await queryInterface.sequelize.transaction(async transaction => {
      ...
      const civilians = await queryInterface.sequelize.query(query, {
        transaction
      });

      if (civilians[1].rowCount > 0) {
        civilians[0].map(async civilian => {
          query = `UPDATE addresses SET civilian_id=${
            civilian.id
          } where addresses.id=${civilian.address_id};`;
          await queryInterface.sequelize.query(query, { transaction });
        });
      }
