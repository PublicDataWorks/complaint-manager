...
const COMPLAINT_TYPE_COLUMN = "complaint_type_id";
...
ADD COLUMN ${COMPLAINT_TYPE_COLUMN} INT,
ADD CONSTRAINT cases_${COMPLAINT_TYPE_TABLE}_fk FOREIGN KEY (${COMPLAINT_TYPE_COLUMN}) references ${COMPLAINT_TYPE_TABLE} (id)`;

const REVERT_CASES = `ALTER TABLE ${CASES_TABLE}
DROP CONSTRAINT IF EXISTS cases_${COMPLAINT_TYPE_TABLE}_fk,
DROP COLUMN IF EXISTS ${COMPLAINT_TYPE_COLUMN}`;

const DROP_VIEW = `DROP VIEW IF EXISTS sortable_cases_view CASCADE`;
...
SELECT cases.id,
   ...
   FROM ((((cases
     ...
     LEFT JOIN ( SELECT tags.id,
            tags.name
           FROM tags) tagdetails ON ((tagdetails.id = casetags.tag_id)))
  GROUP BY cases.id, primary_complainant.complainant_first_name, primary_complainant.complainant_middle_name, primary_complainant.complainant_last_name, primary_complainant.complainant_suffix, primary_complainant.complainant_person_type, primary_complainant.complainant_is_anonymous, case_status.name, case_status.order_key
`;
