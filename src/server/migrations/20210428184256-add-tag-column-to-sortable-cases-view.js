...
const sortableCasesViewWithoutTags = `CREATE VIEW sortable_cases_view AS
          (SELECT id,
                 complaint_type,
                 case_number,
                 YEAR,
...
const sortableCasesViewWithTags = `CREATE VIEW sortable_cases_view AS (
          SELECT
            cases.id,
            array_agg(
    ...
    try {
      await queryInterface.sequelize.transaction(async transaction => {
        await queryInterface.sequelize
          ...
          .then(async () => {
            await queryInterface.sequelize.query(sortableCasesViewWithTags, {
              transaction
            });
          });
      });
    ...
    try {
      await queryInterface.sequelize.transaction(async transaction => {
        await queryInterface.sequelize
          .query(`DROP VIEW ${viewName}`, { transaction })
          .then(async () => {
            await queryInterface.sequelize.query(sortableCasesViewWithoutTags, {
              transaction
            });
          });
      });
