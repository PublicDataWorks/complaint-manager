...
const sortableCaseViewIgnoringDeletedOfficers = `CREATE VIEW sortable_cases_view AS
          (SELECT id,
                 complaint_type,
                 case_number,
                 case_reference,
             ...
             WHERE officers.role_on_case = 'Accused'
               AND officers.created_at =
                 (SELECT MIN(created_at)
                  ...
                  WHERE earliest_officer.role_on_case = 'Accused'
                    AND officers.case_id = earliest_officer.case_id
                    AND earliest_officer.deleted_at is NULL))
                    AS accused_officers
          ON cases.id = accused_officers.case_id
          LEFT JOIN
            (SELECT case_id,
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    try {
        ...
        await queryInterface.sequelize
          ...
          .then(() =>
            queryInterface.sequelize.query(
              sortableCaseViewIgnoringDeletedOfficers,
              {
                transaction
              }
            )
      ...
      throw new Error(
        `Error while adding ignore to deleted accused officers. Internal Error: ${error}`
      );
    }
  },
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      throw new Error(
        `Error while reverting sortable cases view. Internal Error: ${error}`
      );
    }
  }
};
