           ...
           FROM ( SELECT c_1.case_id,
                ...
                UNION ALL
                 SELECT ci.case_id,
                    ...
                    ci.is_anonymous,
                    ci.created_at
                   FROM cases_inmates ci JOIN inmates i ON ci.inmate_id = i.inmate_id
                  WHERE ((ci.role_on_case = 'Complainant') AND (ci.deleted_at IS NULL))
                UNION ALL
                 SELECT ci.case_id,
                 	ci.first_name,
                  ...
                  WHERE ((ci.role_on_case = 'Complainant') AND (ci.inmate_id IS NULL) AND (ci.deleted_at IS NULL))) c
...
const ADD_VALUE_TO_ROLE_ON_CASE_ENUM =
  `DROP VIEW IF EXISTS sortable_cases_view CASCADE;
ALTER TYPE enum_civilians_role_on_case RENAME TO enum_civilians_role_on_case_old;
CREATE TYPE enum_civilians_role_on_case AS ENUM('Complainant', 'Witness', 'Accused');
ALTER TABLE civilians
    ALTER COLUMN role_on_case TYPE enum_civilians_role_on_case
    USING role_on_case::text::enum_civilians_role_on_case;
DROP TYPE enum_civilians_role_on_case_old;` + UPDATE_VIEW;

const REMOVE_VALUE_FROM_ROLE_ON_CASE_ENUM =
  `DROP VIEW IF EXISTS sortable_cases_view CASCADE;
ALTER TYPE enum_civilians_role_on_case RENAME TO enum_civilians_role_on_case_old;
CREATE TYPE enum_civilians_role_on_case AS ENUM('Complainant', 'Witness');
ALTER TABLE civilians
    ALTER COLUMN role_on_case TYPE enum_civilians_role_on_case
    USING role_on_case::text::enum_civilians_role_on_case;
DROP TYPE enum_civilians_role_on_case_old;` + UPDATE_VIEW;
  ...
  up: async (queryInterface, Sequelize) => {
    try {
      await queryInterface.sequelize.transaction(async transaction => {
        await queryInterface.sequelize.query(ADD_VALUE_TO_ROLE_ON_CASE_ENUM, {
          transaction
        });
      });
    } catch (error) {
      throw new Error(
        `Error while adding value to enum type. Internal Error: ${error}`
      );
    }
  },
    ...
    try {
      await queryInterface.sequelize.transaction(async transaction => {
        await queryInterface.sequelize.query(
          REMOVE_VALUE_FROM_ROLE_ON_CASE_ENUM,
          {
            transaction
          }
        );
      ...
      throw new Error(
        `Error while removing enum value. Internal Error: ${error}`
      );
    }
  }
};
