...
const revertSortableCasesView = `CREATE VIEW sortable_cases_view AS
          (SELECT id,
                 complaint_type,
                 case_number,
                 case_reference,
...
const sortableCasesViewWithCnPersonType = `CREATE VIEW sortable_cases_view AS
          (SELECT id,
                 complaint_type,
                 case_number,
                 case_reference,
    ...
    try {
      await queryInterface.sequelize.transaction(async transaction => {
        await queryInterface.sequelize
          ...
          .then(() =>
            queryInterface.sequelize.query(sortableCasesViewWithCnPersonType, {
              transaction
            })
          );
      });
      ...
      throw new Error(
        `Error while adding CN person type to cases sortable view. Internal Error: ${error}`
      );
    }
  },
    ...
    try {
      await queryInterface.sequelize.transaction(async transaction => {
        await queryInterface.sequelize
          .query(`DROP VIEW ${viewName}`, { transaction })
          .then(() =>
            queryInterface.sequelize.query(revertSortableCasesView, {
              transaction
            })
          );
      });
      ...
      throw new Error(
        `Error while removing CN person type from cases sortable view. Internal Error: ${error}`
      );
    }
  }
};
