...
const getCaseHistory = (caseId) => async (dispatch) => {
    ...
    try {
        ...
        const response = await fetch(`${hostname}/api/cases/${caseId}/case-history`, {
            method: 'GET',
            headers: {
        ...
        switch (response.status) {
            ...
            case 401:
                return dispatch(push(`/login`));
            default:
                return dispatch(snackbarError("Something went wrong and we could not fetch the case history."));
        }
    } catch (error) {
        return dispatch(snackbarError("Something went wrong and we could not fetch the case history."));
    }
