...
import ConnectedOfficerDetailsContainer, {OfficerDetailsContainer} from "./OfficerDetailsContainer";
...
describe('OfficerDetailsContainer', () => {
    ...
    test('should redirect and show error if no officer currently selected', () => {
        ...
        mount(
            <Provider store={store}>
                <Router initialEntries={[`/cases/${caseId}/officers/details`]} initialIndex={0}>
                    <Route
        ...
        expect(dispatchSpy).toHaveBeenCalledWith(push(`/cases/${caseId}/officers/search`))
        expect(dispatchSpy).toHaveBeenCalledWith(snackbarError("Please select an officer or unknown officer to continue"))
